...
Developer 产出 `developer_X_plan.json`，声明 `expected_touched_files`。Architect 合并后用 `git diff --name-only` 校验事实来源。
...
│
├─ Phase 1 (并行)
│   Analyst:  KG 查询 → artifacts/impact_report.json
│   Architect:  拆分   → artifacts/plan_tasks.json
│
├─ Phase 2 (Lead 审核)
│   核对两份报告交叉一致性
...
│
├─ Phase 3 (Developer × N, 并行)
│   每个产出: artifacts/developer_X_plan.json + artifacts/developer_X_patch.diff
│   Architect 合并 diff
│
├─ Phase 4 (并行)
│   Auditor:  对照 change_intent → artifacts/audit_report.json (pass/block)
│   Curator:   重新生成 → artifacts/kg_diff.json (run_id 链)
│
└─ 门禁
    Hard block → 拒绝 merge
...
Auditor/Curator 必须在报告里回写引用。默认 30 天过期；永久须显式 `expires_at: null`（防止技术债累积）。
...
软校验的 `edge_budget_by_type` 按**全图**还是**子图**统计？口径不一致会导致 Auditor/Curator 实现分歧。
...
| 10 | 创建 .claude/agents/*.md (6 个角色 prompt 模板) | P2 |
