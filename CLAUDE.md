...
uv run python tools/generate_knowledge_graph.py --json        # JSON 结构化输出
...
uv run python tools/validate_artifacts.py --json artifacts/   # JSON 结构化输出

# 运行测试
uv run pytest
uv run pytest tests/unit/extractors/test_python_extractor.py -k "test_name"
uv run pytest tests/unit/tools/test_config.py
```
...
你（人类）←→ Lead
   ...
   ├─ Phase 1: spawn Analyst → impact_report + plan_tasks
   │           写 checkpoint，等人类确认
   ├─ Phase 2: Gate 判定 → change_intent.json
   │           写 checkpoint，Lead 开始派 Developer
   ├─ Phase 3: spawn Developer × N → 合并 diff
   │           写 checkpoint
   └─ Phase 4: spawn Auditor + Curator → 检查结果
               写 checkpoint = 完成

        ↓ artifacts/ 通信

   Agents（Analyst / Developer / Auditor / Curator）
     ctx 每次重生，隔离干净
```
...
所有 Agent（Analyst / Developer / Auditor / Curator）的输出统一格式：
...
Status: `"ok"` | `"failed"`. Failure 时附带 `failure_type`、`retryable`、`advice`。

Reasoning 每个条目记录一个**决策点**（step/approach/finding/confidence）。Architect 据此判断重试、追问或升级给人类。同一个 Phase 内重试+澄清共享 3 次配额。
...
关键解耦：解析层（语言相关）和生成层（Cypher）通过 15-key dict 契约隔离。生成层不碰任何 AST/tree-sitter 逻辑。

### 目录结构

| 目录/文件 | 作用 |
|-----------|------|
| `tools/` | 11 个模块：7 CLI 工具 + config/errors/neo4j_config + MCP Server |
| `extractors/` | 可插拔的 Extractor 框架（base.py + python_extractor.py + javascript_extractor.py）|
| `queries/<lang>/` | tree-sitter .scm 查询文件（defs, imports, calls, control_flow）：python/javascript/go |
| `artifacts/schemas/` | 7 个 JSON Schema（extractor_output, impact_report, plan_tasks, change_intent, audit_report, kg_diff, overrides）|
| `output/` | 生成的 .cypher 文件 + 存档（`kg_<hash>.cypher`）|
...
| 工具 | 功能 |
|------|------|
| `generate_knowledge_graph.py` | 主入口：自动检测语言 → 调用 Extractor → 生成 Cypher + 存档旧版本（`--ci` / `--json`）|
| `import_neo4j.py` | 清空数据库 → 分批执行 Cypher（约束→节点→关系）→ 统计验证（`--dry-run --json`）|
...
| `validate_artifacts.py` | L1 结构校验 + L2 枚举校验 + L3 Neo4j 交叉引用校验（`--json`）|
...
| `errors.py` | 结构化 JSON 错误输出（错误码 + 统一格式，输出到 stderr）|
...
- `OneFileResult` dataclass 定义了 15 个标准字段
- `extractors/__init__.py` 提供 `detect_language()`（kgflow.toml → 启发式 → fallback python）和 `create_extractor()` 工厂函数
...
- Python >= 3.13，依赖：neo4j, tree-sitter, tree-sitter-javascript
- 包管理：uv（`uv sync` / `uv run`）
- 测试：pytest（`tests/conftest.py` 共享 fixtures）+ dev 依赖 mcp
